...
# MAGIC operational Postgres table, so an app reads fresh, lakehouse-curated data with low latency.
# MAGIC
# MAGIC In this notebook you will:
# MAGIC 1. Build a curated **Delta source** table in Unity Catalog (primary key + Change Data Feed).
# MAGIC 2. Create a **synced table** that replicates it into **your Lakebase branch** as `client_reference`.
...
# MAGIC branch (`USER_BRANCH`) as a Postgres table named `client_reference`.
...
SOURCE_TABLE = f"{UC_CATALOG}.{UC_SCHEMA}.src_client_reference"  # Delta source (lakehouse)
# The synced table registers in UC and materializes in Lakebase as a Postgres table.
SYNCED_TABLE = f"{UC_CATALOG}.{UC_SCHEMA}.client_reference"
PG_TABLE = "client_reference"                                    # the Postgres table name in Lakebase
...
show_view_link("View the Delta source in Unity Catalog",
               uc_table_url(UC_CATALOG, UC_SCHEMA, "src_client_reference"))
...
spec = SyncedTableSpec(
    ...
    branch=f"projects/{PROJECT_ID}/branches/{USER_BRANCH}",  # sync into YOUR branch
    ...
    for attempt in range(1, attempts + 1):
        try:
            op = w.postgres.create_synced_table(
                synced_table=SyncedTable(spec=spec),
            ...
            if "already exists" in msg or "resource_already_exists" in msg:
                try:
                    ...
                    print(f"  stale UC table {SYNCED_TABLE} blocks creation \u2014 dropping it")
                    spark.sql(f"DROP TABLE IF EXISTS {SYNCED_TABLE}")
                    continue
            if attempt == attempts:
                raise
...
for _ in range(30):  # ~a few minutes
    cur = w.postgres.get_synced_table(name=f"synced_tables/{SYNCED_TABLE}")
    state = str(getattr(getattr(cur, "status", None), "detailed_state", "")).upper()
    print("  sync state:", state)
    if any(state.endswith(k) for k in ("ONLINE", "SYNCED", "READY", "ONLINE_NO_PENDING_UPDATE",
                                          "ONLINE_PIPELINE_FAILED", "ONLINE_UPDATING_PIPELINE_RESOURCES")):
        break
...
try:
    with conn.cursor() as cur:
        cur.execute(f"""
            ...
            WHERE schemaname = '{PG_SCHEMA}' ORDER BY 1,2
        """)
        print("Tables on your branch:", [f"{r['schemaname']}.{r['tablename']}" for r in cur.fetchall()])
...
try:
    cur_st = w.postgres.get_synced_table(name=f"synced_tables/{SYNCED_TABLE}")
...
# MAGIC - `client_reference` exists on **your Lakebase branch** and its row count matches the source.
# MAGIC - After raising AC's limit and re-syncing, Lakebase shows **CAD 300,000,000**.
# MAGIC
# MAGIC ## Cleanup (optional)
# MAGIC ```python
# MAGIC # w.postgres.delete_synced_table(name=f"synced_tables/{SYNCED_TABLE}")
...
# MAGIC to your **operational Postgres tables** (e.g. `positions`, `limits` — created directly in Lakebase
# MAGIC by trading apps, **not** the synced copies from this exercise) back into the lakehouse and transform them
# MAGIC into **SCD1** analytical tables via Spark Declarative Pipelines.