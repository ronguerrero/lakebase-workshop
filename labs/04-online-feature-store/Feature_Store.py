...
# autogenerated `cm_<user>` schema, which the facilitator owns you into). Set the catalog
# once in `_setup` — every exercise uses the same one, so there's nothing to configure here.
