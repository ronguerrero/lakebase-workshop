...
> 2. In Unity Catalog, use catalog `main` and schema `cm_features_<my_username>` (sanitize my
...
> 3. Create the offline feature table `main.cm_features_<my_username>.client_risk_features`
...
>    `%client_risk%`.
> 6. Create a `FeatureSpec` `main.cm_features_<my_username>.client_risk_spec` with a
