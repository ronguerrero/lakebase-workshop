...
# Your per-attendee schema name — AUTOGENERATED from your username, and the SAME on both
# sides: Postgres (search_path) and Unity Catalog (feature store + sync/SCD1 tables). The
# facilitator pre-creates it and makes you the owner; you never type it.
_USER_SCHEMA = f"cm_{_sanitize(user_email).replace('-', '_')}"
PG_SCHEMA = _USER_SCHEMA
UC_SCHEMA = _USER_SCHEMA
...
print(f"Schema:   {PG_SCHEMA}   (your schema — same name in Postgres & Unity Catalog)")
print(f"Database: {PG_DATABASE}")
print(f"Catalog:  {UC_CATALOG}   (shared Unity Catalog — Ex4/6/7 write to {UC_CATALOG}.{UC_SCHEMA})")
